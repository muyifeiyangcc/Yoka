...
- (void)yk_configurePage {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(yk_reloadInbox)
                                                 name:YokaLaneLinesDidShiftNotification
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(yk_reloadInbox)
...
- (void)yk_reloadInbox {
    ...
    [[YKLaneVault sharedVault] yk_seedReviewLinesForOwnerKey:ownerKey];
    ...
    for (NSString *peerId in mutualIds) {
        ...
        NSDictionary *preview = [[YKLaneVault sharedVault] yk_previewForOwnerKey:ownerKey peerId:peerId];
        [rows addObject:@{
            @"personaId": peerId,
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![[YKBondLedger sharedLedger] yk_ownerKey:ownerKey isTwinWith:peerId]) {
        [YKCenterToast yk_showNotice:[YKCipherLoom yk_unfurl:@"tHJ77R5VpDfk4eMaeqkiQbcZjEnFkZFfXJJfcX1bmXQ="] inView:self.view];
        return;
    }
