...
- (NSArray<NSDictionary *> *)yk_threadLines {
    ...
    return [[YKLaneVault sharedVault] yk_linesForOwnerKey:[self yk_ownerKey] peerId:self.personaId];
}
...
- (UIImage *)yk_selfAvatar {
    return [[YKRosterVault sharedRoster] yk_portraitImageForActiveMailbox] ?: [UIImage imageNamed:@"headplace"];
}
...
- (void)yk_dispatchComposerText {
    ...
    if (self.personaId.length > 0) {
        ...
        [[YKLaneVault sharedVault] yk_ownerKey:[self yk_ownerKey]
...
- (void)yk_finishRecordingAndSend {
    ...
    NSString *relative = [[YKLaneVault sharedVault] yk_storeVoiceFileFromPath:temp];
    [[NSFileManager defaultManager] removeItemAtPath:temp error:nil];
...
- (void)yk_voiceBubbleTapped:(UITapGestureRecognizer *)gesture {
    ...
    NSString *path = [[YKLaneVault sharedVault] yk_absolutePathForVoiceFile:file];
