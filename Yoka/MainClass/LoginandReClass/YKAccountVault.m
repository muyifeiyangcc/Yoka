...
#import "../RelayClass/YKWhisperVault.h"
#import "../FindClass/YKPublishLedger.h"
#import "../FindClass/YKPieceRevealLedger.h"

NSNotificationName const YokaAccountPresenceDidShiftNotification = @"YokaAccountPresenceDidShiftNotification";

static NSString * const kYKBootPresenceActiveKey = @"yoka.boot.presenceActive.r1";
static NSString * const kYKBootDossierReadyKey = @"yoka.boot.dossierReady.r1";
static NSString * const kYKBootActiveMailboxKey = @"yoka.boot.activeMailbox.r1";
static NSString * const kYKBootActiveAppleIdKey = @"yoka.boot.activeAppleId.r1";
static NSString * const kYKMailboxSecretMapKey = @"yoka.mailbox.secretMap.r1";
static NSString * const kYKTombstoneMailListKey = @"yoka.tombstone.mailList.r1";
static NSString * const kYKTombstoneAppleListKey = @"yoka.tombstone.appleList.r1";
static NSString * const kYKAppleSnapMapKey = @"yoka.apple.personaSnaps.r1";
static NSString * const kYKDossierBundleMapKey = @"yoka.dossier.bundleMap.r1";
static NSString * const kYKPortraitFolderName = @"yoka_face_stash";
static NSString * const kYKAppleAccountPrefix = @"apid:";

@implementation YKAccountVault

+ (instancetype)sharedVault {
    static YKAccountVault *vault = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        vault = [[YKAccountVault alloc] init];
    });
    return vault;
}
...
- (void)yk_postPresenceShift {
    [[NSNotificationCenter defaultCenter] postNotificationName:YokaAccountPresenceDidShiftNotification object:nil];
}
...
- (void)yk_activateMailbox:(NSString *)email {
    NSString *normalized = [YKAccountVault yk_normalizedMailbox:email];
    ...
    if ([YKAccountVault yk_isReviewMailbox:normalized]) {
        ...
        [[YKWhisperVault sharedVault] yk_ensureReviewLinesForOwnerKey:[YKPersonaCatalog yk_reviewPersonaId]];
    } else {
...
- (BOOL)yk_openMailboxSessionWithEmail:(NSString *)email secret:(NSString *)secret errorMessage:(NSString **)errorMessage {
    NSString *normalized = [YKAccountVault yk_normalizedMailbox:email];
    ...
    if ([YKAccountVault yk_isReviewMailbox:normalized]) {
        if (![trimmedSecret isEqualToString:[YKAccountVault yk_reviewSecret]]) {
...
- (void)yk_eraseActiveAccount {
    ...
    if (appleId.length > 0) {
        ...
        if (![YKAccountVault yk_isReviewMailbox:accountKey]) {
            NSMutableDictionary *map = [self yk_mutableSecretMap];
    ...
    if (accountKey.length > 0) {
        ...
        NSString *publishOwner = [YKAccountVault yk_isReviewMailbox:accountKey] ? [YKPersonaCatalog yk_reviewPersonaId] : accountKey;
        [[YKPublishLedger sharedLedger] yk_eraseEntriesForOwnerKey:publishOwner];
        [[YKPieceRevealLedger sharedLedger] yk_eraseRevealsForOwnerKey:publishOwner];
        [[YKPieceRevealLedger sharedLedger] yk_eraseRevealsForOwnerKey:accountKey];
    }
...
- (BOOL)yk_openAppleSessionWithUserId:(NSString *)appleUserId
    ...
    if (email.length > 0) {
        snap[@"email"] = [YKAccountVault yk_normalizedMailbox:email];
    }
...
- (NSString *)yk_displayNameForActiveMailbox {
    ...
    if ([YKAccountVault yk_isReviewMailbox:[self yk_activeMailbox] ?: @""]) {
