...
#import "../LoginandReClass/YKBootNavigator.h"
#import "../LoginandReClass/YKLegalDocViewController.h"
...
- (void)yk_setupViews {
    ...
    UIButton *blacklistButton = [self yk_rowButtonWithTitle:[YKSigilForge yk_unveil:@"97VL1hDna+zNOfXwJ4sORw=="] action:@selector(yk_shadeListTapped:)];
...
- (void)yk_confirmSureTapped:(UIButton *)sender {
    ...
    [self yk_dismissConfirmAnimated:YES completion:^{
        if (kind == YKSettingConfirmKindLogout) {
            [[YKAccountVault sharedVault] yk_closePresence];
            [YKBootNavigator yk_showLandingGate];
            return;
        }
        [YKCenterToast yk_showLoadingInView:weakSelf.view performAfterDelay:0.55 work:^{
            [[YKAccountVault sharedVault] yk_eraseActiveAccount];
            [YKBootNavigator yk_showLandingGate];
        }];
    }];
}
